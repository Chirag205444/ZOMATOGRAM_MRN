...
export default function UserLogin() {
  ...
  const handleSubmit = async(e) => {
    ...
    try{
      const res=await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/auth/user/login`,
  ...
  return (
    <div className="min-h-screen flex items-center justify-center px-4 dark:bg-[radial-gradient(ellipse_at_top,_var(--tw-gradient-stops))] dark:from-[#13383a] dark:to-[#031011]">
      <div className="w-full max-w-md bg-white/80 dark:bg-[#111315] backdrop-blur-sm border border-gray-200 dark:border-white/5 rounded-xl p-8 shadow-lg">
        ...
        <p className="text-sm text-gray-600 dark:text-gray-300 mb-6">Login to continue to your account.</p>

        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1">Email</label>
            <input name='email' required={true} placeholder="you@example.com" className="w-full px-3 py-2 rounded-md border border-gray-300 dark:border-white/5 bg-white dark:bg-[#1a1d21] text-gray-900 dark:text-gray-100 focus:outline-none focus-ring-primary dark:focus:border-[#14b8a6] dark:focus:ring-1 dark:focus:ring-[#14b8a6]" />
          </div>
        ...
        <p className="text-center text-sm text-gray-600 dark:text-gray-300 mt-6">Don't have an account? <Link to="/user/register" className="link-accent dark:text-[#14b8a6] dark:hover:text-[#0d9488] cursor-pointer">Register</Link></p>
      </div>
    </div>
  )
}
